...
2. Виртуальные машины разворачиваются через **terraform**, создание группы безопасности для сети взято из предыдущих заданий
3. Смена имени пользователя по умолчанию, установка ssh-ключа через `cloud-init.yml`. Файл включен в `.gitignore`
4. inventory-файл `prod.yml` будет формироваться из ресурса **yandex_compute_instance.ansible-instance**. Осталось разобраться как передать получившийся адрес хоста clickhouse в переменную endpoint для vector. Вписывать вручную адрес в строку **endpoint: http://62.84.117.148:8123** каждый раз - плохая практика
...
- **Commands should not change things if nothing needs doing** - данное предупреждение **игнориуем**
